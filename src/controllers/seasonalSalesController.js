...
// src/controllers/seasonalSalesController.js
// FIXED VERSION - Added updateSeasonalSale function
const db = require("../config/database");
const redis = require("../config/redis");
const { v4: uuidv4 } = require("uuid");
...
// ✅ NEW: Update seasonal sale (full update)
exports.updateSeasonalSale = async (req, res) => {
  try {
    const { saleId } = req.params;
    const {
      name,
    ...
    // Update seasonal sale main data
    const updates = [];
    const values = [];
    let paramCount = 1;
    ...
    if (bannerColor !== undefined) {
      updates.push(`banner_color = $${paramCount}`);
      values.push(bannerColor);
      paramCount++;
    }
    ...
    if (Array.isArray(productIds) && productIds.length > 0) {
      ...
      for (const productId of productIds) {
        try {
          ...
          if (product.rows.length > 0) {
            const salePrice =
              product.rows[0].price * (1 - (discountPercentage || 15) / 100);
    ...
    res.json({
      success: true,
      message: "Seasonal sale updated successfully",
      seasonalSale: updated.rows[0],
    });
  } catch (error) {
    console.error("Update seasonal sale error:", error);
    res.status(500).json({
      error: "Failed to update seasonal sale",
      details: error.message,
    });
  }
};
